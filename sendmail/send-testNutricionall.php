...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $sql1 = "SELECT * FROM quiz_nutricional WHERE id=?";
    $stmt1 = $conn->prepare($sql1);
    $stmt1->bind_param("i", $idTabla);

  

    if ($stmt1 = $conn->prepare("SELECT * FROM quiz_nutricional WHERE id=?")) {
        $stmt1->bind_param("i", $idTabla);
        if ($stmt1->execute()) {
            ...
            if ($result->num_rows > 0) {
                ...
                if ($stmt3 = $conn->prepare($sql3)) {
                    ...
                    $score=$row["score"];
                    $resul=$row["result_text"];
                    $imc=$row["bmi"];
            // Actualizar el registro con los nuevos datos
            $sql2 = "UPDATE quiz_nutricional SET estado=?, atencion=? WHERE id=?";
                        ...
                        <div style='font-family: Arial, sans-serif; background-color: #f7f7f7; padding: 20px; border-radius: 10px;'>
                            ...
                            <table style='width: 100%; border-collapse: collapse;'>
                                ...
                                <tr style='background-color: #f3f3f3;'>
                                    ...
                                    <td style='padding: 10px; border: 1px solid #ccc;'>$altura</td>
                                </tr>
                                <tr>
                                    <td style='padding: 10px; border: 1px solid #ccc;'><strong>Peso:</strong></td>
                                    <td style='padding: 10px; border: 1px solid #ccc;'>$peso</td>
                                </tr>
                                <tr>
                                    <td style='padding: 10px; border: 1px solid #ccc;'><strong>IMC:</strong></td>
                                    <td style='padding: 10px; border: 1px solid #ccc;'>$imc</td>
                                </tr>
                                   <tr>
                                    <td style='padding: 10px; border: 1px solid #ccc;'><strong>Score:</strong></td>
                                    <td style='padding: 10px; border: 1px solid #ccc;'>$score</td>
                                </tr>
                                   <tr>
                                    <td style='padding: 10px; border: 1px solid #ccc;'><strong>Resultado:</strong></td>
                                    <td style='padding: 10px; border: 1px solid #ccc;'>$resul</td>
                                </tr>
                            </table>
                        </div>
                    </body>
